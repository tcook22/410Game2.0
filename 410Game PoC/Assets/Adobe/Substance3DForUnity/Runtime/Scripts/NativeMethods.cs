...
namespace Adobe.Substance
{
    internal static class NativeMethods
    {
        ...
        internal static uint sbsario_initialize(string pluginPath, string engine_path, IntPtr textureClampExposant, IntPtr memoryBuget)
        {
            lock (_locker)
            {
                try
                {
                    ...
                    return NativeMethodsImpl.sbsario_initialize(engine_path, pluginPath, textureClampExposant, memoryBuget);
#else
                    return NativeMethodsImpl.sbsario_initialize(engine_path, textureClampExposant, memoryBuget);
#endif
                }
        ...
        internal static uint sbsario_sbsar_ensure_unity_format(IntPtr sbsar_handle, IntPtr graph)
        {
            lock (_locker)
            {
                return NativeMethodsImpl.sbsario_sbsar_ensure_unity_format(sbsar_handle, graph);
            }
        }
        ...
        private static class NativeMethodsImpl
        {
            ...
            private delegate uint sbsario_initialize_delegate(IntPtr engine_path, IntPtr textureClampExposant, IntPtr memoryBuget);

            public static uint sbsario_initialize(string engine_path, string pluginPath, System.IntPtr textureClampExposant, IntPtr memoryBuget)
            {
                string myName = System.Reflection.MethodBase.GetCurrentMethod().Name;
                ...
                return function.Invoke(globalStr, textureClampExposant, memoryBuget);
            }
            ...
            //! @return Error type enum representic success or an error
            [DllImport(NativeAssembly)]
            internal static extern uint sbsario_initialize([MarshalAs(UnmanagedType.LPStr)] string engine_path, IntPtr textureClampExposant, IntPtr memoryBuget);
            ...
            private delegate uint sbsario_sbsar_ensure_unity_format_delegate(IntPtr sbsar_handle, IntPtr graph);

            public static uint sbsario_sbsar_ensure_unity_format(IntPtr sbsar_handle, IntPtr graph)
            {
                string myName = System.Reflection.MethodBase.GetCurrentMethod().Name;
                ...
                var function = DLLHelpers.GetFunction(myName, typeof(sbsario_sbsar_ensure_unity_format_delegate)) as sbsario_sbsar_ensure_unity_format_delegate;
                return function.Invoke(sbsar_handle, graph);
            }
            ...
            [DllImport(NativeAssembly)]
            internal static extern uint sbsario_sbsar_ensure_unity_format(IntPtr sbsar_handle, IntPtr graph);
